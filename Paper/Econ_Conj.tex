...
The present design differs because the central behavioral question arises \emph{before} the final choice is made. Respondents do not receive the full profile ex ante. Instead, they first observe the candidate's photograph and then decide whether to unlock party, age, and occupation (among other attributes) under a binding budget and, in some tasks, under time pressure. As a result, the primary quantities of interest are not initially AMCEs of fully displayed attributes on the final choice outcome $Y_{it}$, but quantities governing \emph{information acquisition itself}. Let
\[
U_{ijtm} \in \{0,1\}
\]
...
U_{ijtm} =
\begin{cases}
1 & \text{if respondent } i \text{ unlocks attribute } m
\text{ for profile } j \text{ in task } t,\\
0 & \text{otherwise.}
\end{cases}
\]

The dependent variable in the main analysis is therefore not the realized value of the
...
Let $Z_{ijt}$ denote a profile-level condition that is visible or inferable before the acquisition decision is made. In the present application, $Z_{ijt}$ is candidate gender as inferred from the photograph, but the same framework applies to any pre-acquisition profile condition, such as apparent age \parencite[594]{Keating1999}, facial competence \parencite{Todorov2005,Herrmann2016}, perceived social class \parencite{Bahamonde:2023}, attractiveness \parencite{Banducci2008,Budesheim1994,Dion1972a,Ditonto2018,Hart2011,Kalick1988a,Praino2014b,Praino2019,Stockemer2017,Stockemer2019,Wigginton2021}, or other image-based cues \parencite{Price2011a}. Let $S_{it}\in\{0,1\}$ indicate whether task $t$ is conducted under time pressure. The basic attribute-specific linear probability model is
...
where $L_{ijt}$ indicates the profile's screen position and $f_m(t)$ denotes task-order adjustments, such as a linear task number or task fixed effects. The coefficient $\beta_m$ estimates the difference in the probability of unlocking attribute $m$ associated with the profile-level condition in untimed tasks. In the gender application, $\beta_m$ captures whether female candidates generate greater demand for attribute $m$ than male candidates when respondents are not under time pressure. The coefficient $\lambda_m$ captures the effect of time pressure for the baseline profile condition, while $\gamma_m$ captures whether time pressure changes the profile-condition gap in information demand.

This specification is the regression analogue of the MID estimands defined above. In
particular, when $Z_{ijt}$ and $S_{it}$ are binary,
...
up to the included design controls. Thus, the model directly estimates the level and
contrast quantities implied by the acquisition-stage estimand.
...
\[
\widehat{\beta}_1,\widehat{\beta}_2,\dots,\widehat{\beta}_M,
\]

and, when time pressure is included,

\[
\widehat{\gamma}_1,\widehat{\gamma}_2,\dots,\widehat{\gamma}_M.
\]
...
This requirement rules out a purely expressive preference question. If respondents are asked which candidate they personally prefer (like most conjoint studies do), their answers cannot be treated as right or wrong, effectively ruling out an incentive compatible design. Thus, our design asks respondents to make a prediction rather than to state a preference.

Following \textcite{Antonakis2009a}'s strategy of embedding candidate evaluation in a hypothetical leadership-selection task, and \textcite{Kara-Yakoubian2025}'s strategy of asking respondents to make prospective judgments that can be compared to realized electoral outcomes, our experiment combines a municipal-project frame with an explicit prediction frame. Respondents are not asked which candidate they personally prefer, nor which candidate deserves to be chosen. They are instead asked to predict which candidate local residents would choose for a concrete public role. This makes the response both consequential and incentive compatible: because the profiles correspond to real municipal candidates, respondents' predictions can be tied to payoffs and scored against actual electoral support.


The respondent-facing task is therefore:

\begin{quote}
\emph{Imagine that residents in this municipality had to choose one of these two candidates to lead an important local project. Based on the information available to you, which candidate do you predict residents would choose?}
\end{quote}
